...
export function SSEPanel({ tab, onTabUpdate, onSave, flashSaved, isDirty, availableVariables }: SSEPanelProps) {
  ...
  return (
    <div className="flex h-full flex-col bg-[var(--surface-1)]">
      <ProtocolUrlBar badge="SSE" url={tab.url} placeholder="https://..." disabled={status !== 'disconnected'} onChange={url => onTabUpdate({ url })} status={status} action={status === 'connected' ? <><Square size={13} />Stop</> : status === 'connecting' ? 'Connecting...' : <><Play size={13} />Start</>} onAction={handleStart} onSave={onSave} flashSaved={flashSaved} isDirty={isDirty} availableVariables={availableVariables} />
      <div className="flex items-center gap-2 border-b px-4 py-1" style={{ borderColor: 'var(--border)' }}><span className="text-xs font-medium" style={{ color: 'var(--text-secondary)' }}>Event type:</span><VariableInput variables={availableVariables || []} className="input h-7 max-w-xs text-sm" value={tab.realtime?.eventType || ''} onChange={val => onTabUpdate({ realtime: { ...tab.realtime, eventType: val } })} disabled={status !== 'disconnected'} placeholder="message" /></div>
