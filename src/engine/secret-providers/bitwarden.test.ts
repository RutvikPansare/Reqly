...
function makeMockClient(secrets: Array<{ id: string; key: string; value: string; projectId: string }>, projects: Array<{ id: string; name: string }>) {
  ...
  return {
    ...
    client: {
      ...
      secrets: () => ({
        list: vi.fn().mockResolvedValue({ data: secrets.map(s => ({ id: s.id, key: s.key, organizationId: 'org-1' })) }),
...
describe('BitwardenSecretsProvider', () => {
  const origEnv = process.env.BITWARDENSM_ACCESS_TOKEN;
  ...
  afterEach(() => {
    if (origEnv === undefined) delete process.env.BITWARDENSM_ACCESS_TOKEN;
    else process.env.BITWARDENSM_ACCESS_TOKEN = origEnv;
  });
  ...
  it('uses the env var token over config token', async () => {
    ...
    const provider = new BitwardenSecretsProvider({
      loadConfig: async () => ({ secretProviders: { bitwarden: { accessToken: 'config-token' } } }),
