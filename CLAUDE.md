...
### Task format
Tasks in `docs/todo.md` are flexible. A single line is fine for simple work. Add bullets when Claude needs context to implement without back-and-forth:
...
Reqly is an execution engine, not an AI product. The AI always lives outside Reqly - in the user's Cursor or Claude Code. Reqly's job is to expose reliable, well-typed tools that any AI agent can call.
...
- **No AI logic inside the engine.** The server fires HTTP requests and manages YAML files. It does not call any LLM. There are no LLM calls anywhere in the codebase.
...
- **Config storage:** `~/.reqly/config.json` for global settings
...
1. **Pragmatic TDD** (Red -> Green -> Refactor):
   - **Mandatory for engine and MCP code:** All code in `src/engine/` and `src/mcp/` MUST be developed via TDD. These are the core primitives - they must be correct.
   - **Flexible for UI:** Pure UI components in `src/ui/` may use visual verification.
...
3. **Never write implementation for engine/MCP files before a test exists.**
...
## 4. MCP Tool Contract Rules (MANDATORY)

MCP tool schemas are a public API. AI agents depend on them being stable and well-typed. Breaking a tool's schema silently is the same as breaking a public API.
...
- **Every tool must have a description** that tells an AI agent exactly what it does, what it expects, and what it returns. Agents read these descriptions - they are not optional comments.
- **Return structured, predictable shapes.** Agents parse responses programmatically. Freeform text responses are not acceptable for tool returns.

## 5. General Rules & Personalization
...
## 6. UI Aesthetic (for localhost UI)
