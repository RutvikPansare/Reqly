...
export function WebSocketPanel({ tab, onTabUpdate, onSave }: WebSocketPanelProps) {
  ...
  return (
    <div className="flex h-full flex-col bg-[var(--surface-1)]">
      <ProtocolUrlBar badge="WS" url={tab.url} placeholder="wss://..." disabled={status !== 'disconnected'} onChange={url => onTabUpdate({ url })} status={status} action={status === 'connected' ? 'Disconnect' : status === 'connecting' ? 'Connecting...' : 'Connect'} onAction={handleConnect} saved={Boolean(tab._collection)} onSave={onSave} />
      ...
      {subTab === 'communication' ? <div className="border-b px-4 py-3" style={{ borderColor: 'var(--border)' }}><div className="flex items-start gap-3"><div className="min-h-[104px] flex-1 overflow-hidden rounded" style={{ background: 'var(--surface-2)' }}><CodeMirror value={messageText} onChange={setMessageText} theme="dark" extensions={[json()]} height="104px" basicSetup={{ lineNumbers: false, foldGutter: false }} /></div><button onClick={handleSend} disabled={status !== 'connected' || !messageText} className="btn btn-primary h-8 rounded px-3"><Send size={13} />Send</button></div></div> : <div className="border-b px-4 py-3" style={{ borderColor: 'var(--border)' }}><p className="mb-2 text-xs" style={{ color: 'var(--text-muted)' }}>Subprotocols requested from the server.</p><div className="space-y-2">{protocols.map((value: string, index: number) => <div key={`${index}-${value}`} className="flex items-center gap-2"><input className="input h-8" value={value} onChange={e => updateProtocols(protocols.map((item: string, i: number) => i === index ? e.target.value : item))} placeholder="graphql-ws" /><button onClick={() => updateProtocols(protocols.filter((_: string, i: number) => i !== index))} className="icon-btn" title="Remove protocol"><X size={14} /></button></div>)}</div><button onClick={() => updateProtocols([...protocols, ''])} className="btn btn-secondary mt-3 h-8 rounded px-3"><Plus size={13} />Add protocol</button></div>}
