...
- [x] **T-249** Bitwarden Secrets Manager integration: `bw://` URIs
  - `BitwardenSecretsProvider` in `src/engine/secret-providers/bitwarden.ts`: resolves `bw://project-name/secret-name` via `@bitwarden/sdk-napi` (the specced `@bitwarden/sdk-secrets-manager` package does not exist on npm - see decision log). Token from `BITWARDENSM_ACCESS_TOKEN` env var (wins) or `secretProviders.bitwarden.accessToken` in `~/.reqly/config.json`; missing token gives a clear "Set BITWARDENSM_ACCESS_TOKEN or configure..." error. SDK is lazy-imported and injectable, so tests mock it and the native binding only loads when a `bw://` URI is actually resolved. The SDK requires the organization id on its list calls, so `BITWARDENSM_ORGANIZATION_ID` (or `secretProviders.bitwarden.organizationId`) is also required - caught in self-review, the first cut derived the org id from the list response, which was circular
