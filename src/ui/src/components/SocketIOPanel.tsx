...
interface SocketIOPanelProps { tab: RealtimeTab; onTabUpdate: (updates: Partial<RealtimeTab>) => void; onSave: () => void; }
...
export function SocketIOPanel({ tab, onTabUpdate, onSave }: SocketIOPanelProps) {
  ...
  return (
    <div className="flex h-full flex-col bg-[var(--surface-1)]">
      <ProtocolUrlBar badge="SIO" url={tab.url} placeholder="http://..." disabled={status !== 'disconnected'} onChange={url => onTabUpdate({ url })} status={status} action={status === 'connected' ? 'Disconnect' : status === 'connecting' ? 'Connecting...' : 'Connect'} onAction={handleConnect} saved={Boolean(tab._collection)} onSave={onSave} />
      ...
      <div className="border-b px-4 py-3" style={{ borderColor: 'var(--border)' }}><div className="mb-2 flex items-center gap-2"><span className="text-xs font-medium" style={{ color: 'var(--text-secondary)' }}>Event:</span><input className="input h-8 max-w-xs" value={eventName} onChange={e => setEventName(e.target.value)} placeholder="event name" /></div><div className="flex items-start gap-3"><div className="min-h-[104px] flex-1 overflow-hidden rounded" style={{ background: 'var(--surface-2)' }}><CodeMirror value={messageText} onChange={setMessageText} theme="dark" extensions={[json()]} height="104px" basicSetup={{ lineNumbers: false, foldGutter: false }} /></div><button onClick={handleSend} disabled={status !== 'connected' || !eventName || !messageText} className="btn btn-primary h-8 rounded px-3"><Send size={13} />Emit</button></div></div>
