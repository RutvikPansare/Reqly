...
- [x] **T-262** Fix dead terminal in the bundled desktop app (`posix_spawnp failed`)
  - `terminal.ts`'s `ensureSpawnHelperExecutable` self-heal (chmod +x on node-pty's `spawn-helper` prebuild) called bare `require.resolve` - but the server compiles to ESM (`"type": "module"`), where `require` is a ReferenceError. The function's own best-effort try/catch swallowed it, so the chmod silently never ran. Dev under tsx worked (CJS interop provides `require`); the bundled desktop app did not: `build-desktop-resources.sh`'s fresh staging `npm install` strips the exec bit, the self-heal no-oped, and the first terminal open crashed the bundled server with `Error: posix_spawnp failed` (watchdog respawned it, so it surfaced as a one-off crash in `~/.reqly/desktop.log` rather than a dead app).
  - Fix: `createRequire(import.meta.url)`. Verified end-to-end: stripped the exec bit locally (644), imported the compiled `dist/server/terminal.js`, attached to a throwaway server - bit restored to 755. Also hot-patched the currently installed `/Applications/Reqly.app` bundle's spawn-helper directly so the fix doesn't wait on a rebuild.
...
- [x] **T-257** Finish Electron/agent port isolation - real ephemeral port + readback (completes what T-256 deferred)
  ...
  - **`packages/desktop/src/main.ts`:** replaced the hardcoded `SERVER_URL = 'http://localhost:4242'` with a `resolveElectronPort(expectedPid?, timeoutMs?)` poll against the shared lock file (`~/.reqly/running.json`) - reads the real port the spawned server child wrote after binding. `spawnServerIfNeeded` now returns `Promise<number | null>` (the resolved port) instead of `void`; filters on the *child's* pid (not this Electron process's pid) so a stale lock from an unrelated CLI agent process can't be misread as this spawn's port. New module-level `activePort` tracks the currently-known port; `loadWhenServerReady`, the health watchdog, and the "already running?" reuse check all read/probe it instead of a fixed URL. Watchdog respawns re-resolve `activePort` before the reconnect loop polls again (a respawn lands on a *new* ephemeral port each time).
  - **Why now:** decision-log's T-256 entry explicitly flagged "revisit if a real need for Electron/agent port isolation shows up" - user asked for it directly (running Electron and an agent server simultaneously without one blocking the other's default port).
  - No MCP tool schema, collection format, or `.reqly/` shape changed - internal port-selection wiring only. `tsc --noEmit` clean in root and `packages/desktop`; full suite 1046 green (unchanged pure-function coverage in `lock.test.ts`; `main.ts`'s Electron-process wiring has no unit harness, same as before - verified by inspection + typecheck, consistent with T-256's precedent).

- [x] **T-256** Fix zombie-survives-lock-check bug + wire up `REQLY_ELECTRON` (scope changed from planned two-server port split)
  - **Gap 1 - `REQLY_ELECTRON` never set:** `main.ts`'s `spawnServerIfNeeded` only passed `REQLY_DESKTOP: '1'` to the spawned child, but `index.ts` checked `REQLY_ELECTRON` - the Electron branch (T-255's ephemeral-port + reap logic) was dead code in production; every desktop-spawned server actually went through the agent branch and landed on 4242 by accident. Added `REQLY_ELECTRON: '1'` **alongside** `REQLY_DESKTOP: '1'` in both spawn paths (bundled + resolved-CLI) - kept `REQLY_DESKTOP` untouched since `express.ts`'s login-item routes gate on it specifically; replacing it would have silently killed that feature.
  - **Scope change from the original plan:** the original todo item planned Electron on an OS-assigned ephemeral port (like T-255 originally built). That would have broken the app - `main.ts`'s `SERVER_URL` and `probeServer()`/`loadURL()` are all hardcoded to `localhost:4242`, and nothing read the real port back out of the lock file into the window. Turning on `REQLY_ELECTRON` alone would have made Electron spawn a server on a random port its own window could never reach. Decided against building the port-readback plumbing (see decision-log) - unified `index.ts`'s electron/agent branches into one path that always targets 4242, distinguished only by lock `type` for reap/fallback decisions. Electron and agent now: reap a stale lock of their *own* kind (`shouldReapStaleLock`, from T-255), or fall back to an OS-assigned port if a *live* lock of the *other* kind already holds 4242 (`shouldFallbackToEphemeralPort`, new in `lock.ts`) - avoids racing into `EADDRINUSE`.
  - **Gap 2 - zombie pid passed the lock check, HTTP probe never ran (`main.ts`):** `lockFilePointsToLiveServer() || await probeServer()` short-circuited on a live pid alone. A zombie (process alive, port never bound - e.g. it lost a startup race) passed `process.kill(pid, 0)`, so the probe never ran, Electron skipped spawning, and the window sat in a connection-refused reconnect loop against a server that never existed. Removed the short-circuit - `alreadyRunning` is now just `await probeServer()`, the only signal that actually means "serving." Deleted the now-dead `lockFilePointsToLiveServer` helper.
  - TDD (`lock.test.ts`): `shouldFallbackToEphemeralPort` covers no-lock, dead-pid, same-type-live (reaps instead), and both other-type-live directions. Full suite 1046 green, `tsc --noEmit` clean in both the root and `packages/desktop`.

## 2026-07-08

- [x] **T-255** Reap stale server processes on startup (Electron + agent)
  - Renumbered from a duplicate T-253 found queued in `todo.md` (T-253 was already used by the 2026-07-06 UI audit entry below) - IDs must never repeat.
  - **Root cause:** Electron's startup path grabbed an ephemeral port without ever reading the lock file, so a crashed/force-quit Electron launch left its spawned server child running invisibly; the next launch just overwrote the lock and orphaned it. The agent branch had a partial fix (SIGTERM + blind 600ms wait) that let slow-exiting processes survive and pile up the same way. Confirmed live: found 4 orphaned `reqly start` processes plus a lock file pointing at a dead pid.
  - **`killWithEscalation(pid)`** (`src/server/lock.ts`): SIGTERM, then polls `isProcessAlive` every 100ms up to a 2s timeout, escalating to SIGKILL if the process hasn't exited. Replaces the blind wait in the agent branch.
  - **`shouldReapStaleLock(existing, myLockType)`** (`src/server/lock.ts`): shared pure decision used by both startup branches - only reaps a live stale lock of the *same* kind (Electron reaps a dead Electron lock, agent reaps a dead agent lock); a live lock of the other kind is left alone and the caller falls back to a different port. Keeps the Electron-vs-agent distinction from drifting between the two call sites.
  - Electron branch (`src/server/index.ts`) now reads the lock and reaps a stale Electron-owned process before binding its ephemeral port.
  - TDD: `lock.test.ts` covers SIGTERM-only exit resolving fast, SIGKILL escalation after timeout (spawned against real child processes), a dead pid resolving cleanly, and all six `shouldReapStaleLock` branch combinations. Full suite 1041 green, `tsc --noEmit` clean.
