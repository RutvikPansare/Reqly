    ...
    <div className="absolute inset-0 bg-gray-950 z-50 flex flex-col p-6 animate-slide-up overflow-hidden">
      <div className="flex justify-between items-center mb-6 border-b border-gray-800 pb-4 shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            Stop on failure
            <span className={`relative inline-flex h-5 w-9 items-center rounded-full border transition-colors ${stopOnFailure ? 'bg-blue-600 border-blue-600' : 'bg-gray-800 border-gray-700'}`}>
          ...
          <button
            className="inline-flex items-center justify-center w-8 h-8 rounded-lg border border-gray-800 text-gray-500 hover:text-white hover:border-gray-600 transition-colors"
            onClick={onClose}
          >
      ...
      {result && (
        <div className="mb-6 p-4 rounded-lg bg-gray-900 border border-gray-800 flex justify-between items-center shrink-0">
          ...
          <button
            onClick={runCollection}
            className="inline-flex items-center gap-1.5 text-sm text-blue-400 hover:text-blue-300 px-3 py-1.5 rounded-lg border border-gray-800 hover:border-blue-800 transition-colors"
          >
      ...
      <div className="flex-1 min-h-0 overflow-y-auto space-y-3 pb-12">
        {result?.results?.map((req: any, i: number) => (
          <div key={i} className={`p-4 rounded-lg border min-w-0 ${req.passed ? 'bg-gray-900 border-gray-800' : 'bg-red-950/20 border-red-900/50'}`}>
