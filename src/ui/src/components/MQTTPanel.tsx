...
export function MQTTPanel({ tab, onTabUpdate, onSave, flashSaved, isDirty, varCompletionExtension, availableVariables }: MQTTPanelProps) {
    ...
    <div className="h-full overflow-auto">
      <div className="border-b" style={{ borderColor: 'var(--border)' }}>
        <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-1.5 text-sm">
          <label className="flex items-center gap-2"><span style={{ color: 'var(--text-secondary)' }}>Client ID:</span><VariableInput variables={availableVariables || []} className="input h-7 w-48" value={tab.realtime?.mqttClientId || ''} onChange={val => updateRealtime({ mqttClientId: val })} placeholder="Auto-generated" disabled={status !== 'disconnected'} /></label>
      ...
      <div className="grid md:grid-cols-2" style={{ borderBottom: '1px solid var(--border)' }}>
        <div className="border-b px-4 py-3 md:border-b-0 md:border-r" style={{ borderColor: 'var(--border)' }}>
          ...
          <div className="mb-2 flex gap-2"><VariableInput variables={availableVariables || []} className="input h-8" value={subTopic} onChange={setSubTopic} onKeyDown={(e: any) => e.key === 'Enter' && handleSubscribe()} placeholder="sensors/#" /><button onClick={handleSubscribe} disabled={!subTopic} className="btn btn-primary h-8 rounded px-3">Subscribe</button></div>
        ...
        <div className="px-4 py-3">
          ...
          <VariableInput variables={availableVariables || []} className="input mb-2 h-8" value={publishTopic} onChange={setPublishTopic} placeholder="topic/name" />
          <div className="min-h-[80px] overflow-hidden rounded" style={{ background: 'var(--surface-2)' }}><CodeMirror value={publishMessage} onChange={setPublishMessage} theme="dark" extensions={[json(), varCompletionExtension]} height="80px" basicSetup={{ lineNumbers: false, foldGutter: false }} className="[&_.cm-scroller]:overflow-auto" /></div>
  ...
  return (
    <div className="flex h-full flex-col bg-[var(--surface-1)]">
      <ProtocolUrlBar badge="MQTT" url={tab.url} placeholder="ws:// or wss://..." disabled={status !== 'disconnected'} onChange={url => onTabUpdate({ url })} status={status} action={status === 'connected' ? 'Disconnect' : status === 'connecting' ? 'Connecting...' : 'Connect'} onAction={handleConnect} onSave={onSave} flashSaved={flashSaved} isDirty={isDirty} availableVariables={availableVariables} />
