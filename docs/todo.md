...
- [ ] **T-086** Publish Reqly to npm
  - _in progress: pre-publish checklist done - `npm test` (308/308 pass), root build verified (dist/ + shebang), `npm pack --dry-run` checked. Found and fixed a real packaging bug: `.npmignore`'s `src/` rule wasn't excluding `src/ui/node_modules` (8516 files, 149.7MB unpacked) - replaced with a `files` allowlist in package.json (`dist`, `packages/reqly-middleware/src/{core,index,next}.ts`, `packages/reqly-middleware/package.json`, `README.md`, `llms.txt`), fixed `main` to point at `dist/server/index.js` (was `src/server/index.ts`, which won't exist in the published package), and moved `tsx`/`typescript`/`vitest` from `dependencies` to `devDependencies` (unused by the built CLI at runtime). Tarball now packs to 368.7kB / 155 files. Still needed: `npm install -g .` smoke test, `reqly setup cursor` config check, then `npm login` + `npm publish --access public` - deferred, needs Rutvik's npm credentials and explicit go-ahead (publishing is irreversible/public)._
