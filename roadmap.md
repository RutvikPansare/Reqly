...
- [ ] **Mock server** - `reqly mock <collection>` serves saved example responses as a real HTTP server on a configurable port (default 4243). Route matching on method + URL pattern; falls back to 404 with a helpful error if no example exists. Response delay configurable per example to simulate latency. Agents can spin up a mock via the `start_mock` MCP tool and tear it down with `stop_mock`. UI shows mock status in the nav rail with active route count. Enables frontend development against a backend that isn't ready, and lets agents test client code against controlled responses. Collections already store example responses (T-084) - the mock server just serves them. Tasks: T-101 through T-103.

- [ ] **.env file integration** - Reqly automatically reads a `.env` file in the project root and makes every key available as a variable in all requests, resolved at the lowest priority (collection vars > env vars > .env file). Secrets stay out of the YAML that travels with git. Zero config - if `.env` exists, it's loaded. Hot-reloads on file change. Variables tab in the request editor shows `.env` as the source. MCP tool `get_variables` includes `.env` keys with `source: ".env"`. Agents can reference `{{STRIPE_SECRET_KEY}}` without ever writing the value into a collection file. Tasks: T-104.

- [ ] **OpenAPI contract validation** - after any request is fired, optionally validate the response against an OpenAPI spec (pointed at via a file path or URL in collection settings). Reports: missing required fields, wrong types, unexpected status codes, schema violations. Shown as a new "Contract" tab in the response viewer alongside Assertions. MCP tool `validate_response` returns structured violations so agents can catch contract breaks in CI. `reqly run <collection> --validate-spec <path>` flag for CLI. Tasks: T-105 through T-106.
