export type NavPanel = 'collections' | 'environments' | 'history' | 'capture' | 'settings';
...
// Icons: 20px, stroke-based (lucide-style) to match Hoppscotch sizing.
// Active state uses a left accent bar (Hoppscotch `after:left-0 after:w-0.5` pattern).
      ...
      {NAV_ITEMS.map(item => {
        ...
        return (
          ...
          >
            ...
            {isActive && (
              <span className="absolute left-0 inset-y-0 w-0.5 bg-blue-500" aria-hidden="true" />
            )}
            {item.icon}
          </button>
        );
